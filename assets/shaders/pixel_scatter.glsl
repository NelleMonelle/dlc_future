...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    float x_offset = (random(seed * coords_floored      ) - 0.5) * 2.0 * max_dist / texture_size.x;
    float y_offset = (random(seed * coords_floored * 1.1) - 0.5) * 2.0 * max_dist / texture_size.y;
