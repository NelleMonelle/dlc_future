local spell, super = Class(Spell, "ultra_break")
...
function spell:init()
    ...
    self.effect = "Break\nDefenses"
    -- Menu description
    self.description = "Damages the enemy and breaks their defense."
    -- Check description
    self.check = "Damages the enemy and breaks their defense."
...
function spell:onCast(user, target)
	...
	Game.battle.timer:after(0.1/2, function()
		...
		if not target.unbreakable then
			target.defense = 0
			Assets.playSound("bell_dead")
			target:statusMessage("msg", "broken", {1, 1, 1})
		end
	end)
end
